...
use crate::plonky2_verifier::{chip::goldilocks_chip::GoldilocksChipConfig, context::RegionCtx};
...
impl<F: PrimeField> CustomGateConstrainer<F> for U32ArithmeticGateConstrainer {
        ...
        for i in 0..self.num_ops {
            ...
            let combined_output = {
                let base = goldilocks_extension_chip
                    .constant_extension(ctx, &[GoldilocksField::from_canonical_u64(1 << 32u64), GoldilocksField::ZERO])?;
                let one = goldilocks_extension_chip.one_extension(ctx)?;
                let u32_max = goldilocks_extension_chip.constant_extension(
                    ctx,
                    &[GoldilocksField::from_canonical_u64(u32::MAX as u64), GoldilocksField::ZERO],
                )?;
            ...
            let base = goldilocks_extension_chip.constant_extension(
                ctx,
                &[
                    GoldilocksField::from_canonical_u64(1u64 << Self::limb_bits()),
            ...
            for j in (0..Self::num_limbs()).rev() {
                ...
                for x in 0..max_limb {
                    let x = goldilocks_extension_chip
                        .constant_extension(ctx, &[GoldilocksField::from_canonical_u64(x), GoldilocksField::ZERO])?;
