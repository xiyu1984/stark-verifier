...
#[rustfmt::skip]
const FAST_PARTIAL_FIRST_ROUND_CONSTANT: [u64; T]  = [
...
#[rustfmt::skip]
const FAST_PARTIAL_ROUND_CONSTANTS: [u64; R_P]  = [
...
#[rustfmt::skip]
const FAST_PARTIAL_ROUND_VS: [[u64; T - 1]; R_P] = [
...
#[rustfmt::skip]
    const FAST_PARTIAL_ROUND_W_HATS: [[u64; T - 1]; R_P] = [
...
#[rustfmt::skip]
    ...
    const FAST_PARTIAL_ROUND_INITIAL_MATRIX: [[u64; T - 1]; T - 1] = [
...
impl PoseidonGateConstrainer {
    ...
    pub const WIRE_SWAP: usize = 2 * T;

    const START_DELTA: usize = 2 * T + 1;
    ...
    fn wire_full_sbox_0(round: usize, i: usize) -> usize {
        ...
        debug_assert!(round < R_F_HALF);
        debug_assert!(i < T);
        Self::START_FULL_0 + T * (round - 1) + i
    }

    const START_PARTIAL: usize = Self::START_FULL_0 + T * (R_F_HALF - 1);
    ...
    fn wire_partial_sbox(round: usize) -> usize {
        debug_assert!(round < R_P);
        Self::START_PARTIAL + round
    }

    const START_FULL_1: usize = Self::START_PARTIAL + R_P;
    ...
    fn constant_layer<F: PrimeField>(
        ...
        state: &mut Vec<AssignedExtensionFieldValue<F, 2>>,
        round_ctr: usize,
    ) {
        ...
        for i in 0..T {
            let c = goldilocks_extension_chip
                ...
                .unwrap();
            state[i] = goldilocks_extension_chip
                .add_extension(ctx, &state[i], &c)
                .unwrap();
        }
    }
    ...
    fn sbox<F: PrimeField>(
        ...
        ctx: &mut RegionCtx<'_, F>,
        goldilocks_chip_config: &GoldilocksChipConfig<F>,
        element: &AssignedExtensionFieldValue<F, 2>,
    ...
    fn sbox_layer<F: PrimeField>(
        ...
        for i in 0..T {
            state[i] = self.sbox(ctx, goldilocks_chip_config, &state[i]).unwrap();
        }
    }
    ...
    fn mds_partial_layer_init<F: PrimeField>(
        ...
        for _ in 0..T {
            result.push(goldilocks_extension_chip.zero_extension(ctx).unwrap());
        }
        ...
        for r in 1..T {
            for c in 1..T {
                let t = goldilocks_extension_chip
                    ...
                    .unwrap();
                result[c] = goldilocks_extension_chip
                    .mul_add_extension(ctx, &t, &state[r], &result[c])
                    .unwrap();
            }
        }
        result
...
impl<F: PrimeField> CustomGateConstrainer<F> for PoseidonGateConstrainer {
    fn eval_unfiltered_constraint(
        ...
        let swap = &local_wires[Self::WIRE_SWAP];
        constraints.push(goldilocks_extension_chip.mul_sub_extension(ctx, swap, swap, swap)?);

        // Assert that each delta wire is set properly: `delta_i = swap * (rhs - lhs)`.
        for i in 0..4 {
            let input_lhs = &local_wires[Self::wire_input(i)];
            let input_rhs = &local_wires[Self::wire_input(i + 4)];
            let delta_i = &local_wires[Self::wire_delta(i)];
            let diff = goldilocks_extension_chip.sub_extension(ctx, input_rhs, input_lhs)?;
            constraints
                .push(goldilocks_extension_chip.mul_sub_extension(ctx, swap, &diff, delta_i)?);
        }

        // Compute the possibly-swapped input layer.
        let mut state = Vec::with_capacity(T);
        for _ in 0..T {
            state.push(goldilocks_extension_chip.zero_extension(ctx)?);
        }
        for i in 0..4 {
            let delta_i = &local_wires[Self::wire_delta(i)];
            let input_lhs = &local_wires[Self::wire_input(i)];
            let input_rhs = &local_wires[Self::wire_input(i + 4)];
            state[i] = goldilocks_extension_chip.add_extension(ctx, input_lhs, delta_i)?;
            state[i + 4] = goldilocks_extension_chip.sub_extension(ctx, input_rhs, delta_i)?;
        }
        for i in 8..T {
            state[i] = local_wires[Self::wire_input(i)].clone();
        }
        ...
        for r in 0..R_F_HALF {
            ...
            if r != 0 {
                for i in 0..T {
                    let sbox_in = &local_wires[Self::wire_full_sbox_0(r, i)];
                    constraints
                        .push(goldilocks_extension_chip.sub_extension(ctx, &state[i], sbox_in)?);
                    state[i] = sbox_in.clone();
                }
            }
            self.sbox_layer(ctx, goldilocks_chip_config, &mut state);
            state = self.mds_layer(ctx, goldilocks_chip_config, &state);
            round_ctr += 1;
        }
        ...
        let sbox_in = &local_wires[Self::wire_partial_sbox(R_P - 1)];
        constraints.push(goldilocks_extension_chip.sub_extension(ctx, &state[0], &sbox_in)?);
        state[0] = self.sbox(ctx, goldilocks_chip_config, sbox_in)?;
        state = self.mds_partial_layer_fast(ctx, goldilocks_chip_config, &state, R_P - 1);
        round_ctr += R_P;
        ...
        for r in 0..R_F_HALF {
            self.constant_layer(ctx, goldilocks_chip_config, &mut state, round_ctr);
            for i in 0..T {
                let sbox_in = &local_wires[Self::wire_full_sbox_1(r, i)];
                constraints.push(goldilocks_extension_chip.sub_extension(ctx, &state[i], sbox_in)?);
                state[i] = sbox_in.clone();
            }
            self.sbox_layer(ctx, goldilocks_chip_config, &mut state);
            state = self.mds_layer(ctx, goldilocks_chip_config, &state);
        ...
        for i in 0..T {
            constraints.push(goldilocks_extension_chip.sub_extension(
                ctx,
                &state[i],
                &local_wires[Self::wire_output(i)],
            )?);
        }
